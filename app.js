...
const REMOTE_MODE = window.__TODO_REMOTE__ === true || Boolean(window.google?.script?.run);
const DEMO_MODE = !REMOTE_MODE && new URLSearchParams(window.location.search).get('demo') === '1';
const STORAGE_KEY = DEMO_MODE ? 'todo-portfolio-demo-tasks-v1' : 'todo-beta-tasks';

function createSampleTasks() {
  return [
  {
    id: crypto.randomUUID(),
    title: 'Revisar o protótipo do painel',
    nextAction: 'Validar os filtros e a hierarquia dos cards',
    status: 'Em andamento',
    priority: 'Alta',
    schedule: futureDateTime(1, 10),
    deadline: '',
    link: 'https://example.com/prototipo',
    dependency: '',
    tags: ['produto', 'UX'],
    color: 'blue',
    contacts: [{ name: 'Marina', email: 'marina@example.com', phone: '', whatsapp: '' }],
    order: 0,
    createdAt: Date.now(),
    updatedAt: Date.now(),
    completedAt: null
  ...
  {
    id: crypto.randomUUID(),
    title: 'Confirmar conteúdo da apresentação',
    nextAction: 'Enviar a última versão e registrar o retorno',
    status: 'Aguardando',
    priority: 'Média',
    schedule: '',
    deadline: futureDate(3),
    link: '',
    dependency: 'Aguardando aprovação',
    tags: ['portfólio', 'conteúdo'],
    color: 'sand',
    contacts: [{ name: 'Carlos', email: '', phone: '', whatsapp: '5521999999999' }],
    order: 1,
  ...
  {
    id: crypto.randomUUID(),
    title: 'Planejar a próxima entrega',
    nextAction: 'Quebrar o escopo em tarefas menores',
    status: 'Fazer',
    priority: 'Baixa',
    schedule: '',
...
function loadTasks() {
  ...
  return saved ? JSON.parse(saved) : createSampleTasks();
}
...
async function init() {
  ...
  el('demo-badge').hidden = !DEMO_MODE;
  el('demo-reset').hidden = !DEMO_MODE;
  if (REMOTE_MODE) {
    el('app').hidden = true;
    ...
    el('ai-submit').disabled = !DEMO_MODE;
    el('ai-help').textContent = DEMO_MODE
      ? 'Demonstração local: a interpretação abaixo é simulada e não envia dados para uma IA.'
      : 'A criação por IA estará disponível na versão privada do Apps Script.';
    renderTasks();
  }
...
function bindEvents() {
  ...
  el('demo-reset').addEventListener('click', resetDemo);
...
async function createWithAi() {
  if (!REMOTE_MODE && !DEMO_MODE) return;
  ...
  try {
    const generated = DEMO_MODE
      ? simulateTaskFromPrompt(prompt)
      : await callServer('createTaskFromPrompt', state.token, prompt);
    fillForm({ ...emptyTask(), ...generated, id: '' });
    el('ai-prompt').value = prompt;
    el('ai-help').textContent = DEMO_MODE
      ? 'Simulação concluída. Revise os campos antes de salvar.'
...
function resetDemo() {
  if (!DEMO_MODE || !confirm('Restaurar as tarefas originais da demonstração?')) return;
  localStorage.removeItem(STORAGE_KEY);
  state.tasks = createSampleTasks();
  state.showCompleted = false;
  state.sort = 'manual';
  document.querySelector('input[name="sort"][value="manual"]').checked = true;
...
function simulateTaskFromPrompt(prompt) {
  const normalized = normalize(prompt);
  const hourMatch = normalized.match(/\b([01]?\d|2[0-3])h(?::?([0-5]\d))?\b/);
  const weekdayNames = ['domingo', 'segunda', 'terca', 'quarta', 'quinta', 'sexta', 'sabado'];
  let daysAhead = normalized.includes('amanha') ? 1 : normalized.includes('hoje') ? 0 : null;
  if (daysAhead === null) {
    const targetDay = weekdayNames.findIndex(day => normalized.includes(day));
    if (targetDay >= 0) daysAhead = (targetDay - new Date().getDay() + 7) % 7 || 7;
  }

  const schedule = daysAhead !== null || hourMatch
    ? futureDateTime(daysAhead || 0, Number(hourMatch?.[1] || 9), Number(hourMatch?.[2] || 0))
    : '';
  const compactTitle = prompt.split(/[,.!?]/)[0].trim().slice(0, 72) || 'Nova tarefa';

  return {
    title: compactTitle.charAt(0).toUpperCase() + compactTitle.slice(1),
    nextAction: prompt,
    status: 'Fazer',
    priority: /urgente|importante|prioridade alta/.test(normalized) ? 'Alta' : 'Média',
    schedule,
    deadline: '',
    link: prompt.match(/https?:\/\/\S+/i)?.[0] || '',
    dependency: '',
    tags: ['demo', 'IA simulada'],
    color: 'lavender',
    contacts: []
  };
}
...
function futureDate(daysAhead) {
  ...
  return localDateParts(date).slice(0, 3).join('-');
}

function futureDateTime(daysAhead, hour, minute = 0) {
  const date = new Date();
  date.setDate(date.getDate() + daysAhead);
  date.setHours(hour, minute, 0, 0);
  const [year, month, day, hours, minutes] = localDateParts(date);
